...
async def save_userinfo(self, user_input: str):
    # Step 1: Use the LLM to decide if the input is important
    decision_prompt = (
    ...
    # Step 2: Conditionally save input to ChromaDB
    if decision_text == "yes":
        doc_id = f"analyzed_memory_{hash(user_input)}"
        await add_document(user_input, doc_id, self.collection)
